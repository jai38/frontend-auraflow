...
export function WriteFlowTab() {
  ...
  useEffect(() => {
    ...
    const hasUsed = localStorage.getItem("hasUsedWriteFlow") === "true";
    ...
    localStorage.setItem("hasUsedWriteFlow", "true");
    ...
    const fetchSessionLimits = async () => {
      try {
        ...
        console.error("Failed to initialize WriteFlow session limits:", err);
      }
    };
    fetchSessionLimits();
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error("WriteFlow chat request failed:", err);
      setMessages((prev) => [
        ...prev,
        {
          sender: "model",
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)] min-h-[500px] bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden font-sans">
      ...
      <header className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 font-serif leading-none">WriteFlow</h2>
